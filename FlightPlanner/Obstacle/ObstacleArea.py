...
from FlightPlanner.types import Point3D, TurnDirection, ObstacleAreaResult, IntersectionStatus, Point3dCollection
...
#         {
    ...
    def imethod_0(self, point3d, tolerance):
        return self.pointInArea(point3d, tolerance)
...
class SecondaryAreaPie(ISecondaryArea):
    ...
    def __init__(self, point3d_0, point3d_1, point3d_2):
        ...
        polylineArea = PolylineArea()
        polylineArea.Add(PolylineAreaPoint(point3d_0, num))
        polylineArea.Add(PolylineAreaPoint(point3d_2))
        pointCount = len(polylineArea.method_14())
...
class SecondaryObstacleAreaWithManyPoints:
    def __init__(self, polylineArea, primaryPolyline, bothPolylineArea = False, standardDist = None):
        #   primaryPolyline : PolylineArea

        self.area = polylineArea
        self.PreviewArea = PolylineArea(polylineArea.method_14_closed())
        self.primaryPolyline = primaryPolyline
        self.polygonArea = self.area.method_14_closed()
        self.error = None
        self.nominalTrack = 0.0
        self.bothPolylineArea = bothPolylineArea
        self.standardDist = standardDist
    def pointInPolygon(self, point3d, tolerance):
        return self.pointInArea(point3d, tolerance)

    def pointInArea(self, point3d, tolerance):
        return MathHelper.pointInPolygon(self.polygonArea, point3d, tolerance)
    def imethod_1(self, point3d_0, double_0, double_1, returnList):
        double_2 = None
        double_3 = None
        if not self.pointInPolygon(point3d_0, double_0):
            returnList.append(double_2)
            returnList.append(double_3)
            return ObstacleAreaResult.Outside
        pointArray = self.primaryPolyline.method_14()
        # pointsInArea = []
        # for i in range(len(self.polygonArea)):
        #     if i == 0:
        #         continue
        #     line = QgsGeometry.fromPolyline([self.polygonArea[i], self.polygonArea[i-1]])
        #     point1, point2 = MathHelper.getVerticalLine([self.polygonArea[i-1], self.polygonArea[i]], point3d_0, 50000)
        #     verticalLine = QgsGeometry.fromPolyline([point1, point2])
        #     if line.intersects(verticalLine):
        #         intersectGeom = line.intersection(verticalLine)
        #         pointsInArea.append(intersectGeom.asPoint())
        otherIntersectionPoint = None
        for i in range(len(pointArray)):
            if i == 0:
                continue
            line = QgsGeometry.fromPolyline([pointArray[i], pointArray[i-1]])
            point1, point2 = MathHelper.getVerticalLine([pointArray[i-1], pointArray[i]], point3d_0, 50000)
            verticalLine = QgsGeometry.fromPolyline([point1, point2])
            if line.intersects(verticalLine):
                intersectGeom = line.intersection(verticalLine)
                # intersectPoint = intersectGeom.asPoint()
                intersectPoint = MathHelper.getIntersectionPoint(pointArray[i], pointArray[i-1], point1, point2)
                if self.bothPolylineArea:
                    for j in range(len(self.polygonArea)):
                        if j == len(self.polygonArea) - 1:
                            break
                        l = QgsGeometry.fromPolyline([self.polygonArea[j], self.polygonArea[j+ 1]])
                        if l.intersects(verticalLine):
                            otherIntersectGeom = l.intersection(verticalLine)
                            # otherIntersectionPoint = otherIntersectGeom.asPoint()
                            otherIntersectionPoint = MathHelper.getIntersectionPoint(self.polygonArea[j], self.polygonArea[j+ 1], point1, point2)

                            break
                else:
                    for j in range(len(pointArray) - 1, len(self.polygonArea)):
                        if j == len(self.polygonArea) - 1:
                            break
                        l = QgsGeometry.fromPolyline([self.polygonArea[j], self.polygonArea[j+ 1]])
                        if l.intersects(verticalLine):
                ...
                dist = MathHelper.calcDistance(point3d_0, intersectPoint)
                if MathHelper.calcDistance(otherIntersectionPoint, intersectPoint) == 0:
                    if self.standardDist != None:
                        double_2 = double_1 * (1 - MathHelper.calcDistance(point3d_0, intersectPoint) / self.standardDist)
                    else:
                        double_2 = double_1
                else:
                    double_2 = double_1 * (1 - MathHelper.calcDistance(point3d_0, intersectPoint) / MathHelper.calcDistance(otherIntersectionPoint, intersectPoint))
                double_3 = dist
                returnList.append(double_2)
                returnList.append(double_3)
                return ObstacleAreaResult.Secondary
    ...
    IsValidForPreview = property(isValidForPreview, None, None, None)
    def ToString(self):
        return "SecondaryAreaWithManyPoints (%s points) (point count of PrimaryPolyline : %s)"%(str(len(self.area)), str(len(self.primaryPolyline)))
